...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length() && (count.load()<g_loop_count || g_loop_count==0); ++i) {
		// printf("count = %d\n", count.load());
...
/* 发布者函数 */
extern "C" int publisher_main(int domainId, int sample_count, size_t data_size)
{
	...
	instance->number = 0;
	instance->data = new char[data_size+1];
	for(int i=0;i<data_size;i++) {
		instance->data[i]='a';
	}
	instance->data[data_size]=0;
	...
	/* for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		...
		instance->data = new char[data_size];
		char payload[]="a"; // 载荷

		instance->data[0]=0; // 初始化
		for(int i=0;i<data_size;i++){ // 填充数据
			strcat(instance->data, payload);
		}
	...
	/* 9. 删除所有实例 */
	auto status = publisher_shutdown(participant);
	delete reader_listener;
	return status;
}
...
{
	size_t data_size = 1024;	// 数据大小默认为1kB
	int domain_id = 0;
	int sample_count = 0; /* 无限循环 */

	if(argc >=2 ) {
		sample_count = atoi(argv[1]); // 循环次数
	}
	if (argc >= 3) {
		data_size = atoi(argv[2]);  /* 数据大小 */
	}
	if (argc >= 4) {
		domain_id = atoi(argv[3]); /* 发送至域domain_id */
	}

	auto status = publisher_main(domain_id, sample_count, data_size);
