...
atomic_ullong io {0}; // 1秒内接收到的数据数据包数量
atomic_ullong count {0}; // 总共接收到的数据包数量
atomic_ullong number {1}; // 最新收到的数据包编号
atomic_ullong data_size {0}; // 收到的数据包大小
const long mb = pow(2,20);

void display(int) {
	alarm(1);
	printf("count=%ld number=%ld\n", count.load(), number.load());
	printf("吞吐量: %ld Mbps\n", io * data_size * 8 / mb);
	printf("丢包率: %lf %%\n", (1-(double)count/number)*100);
	io=0;
}
...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length(); ++i) {
		count++;
		io++;
		number = data_seq[i].number;
		data_size = data_seq[i].size;
	}
}
